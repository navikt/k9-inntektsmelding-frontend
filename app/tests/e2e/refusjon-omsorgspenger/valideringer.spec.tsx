...
test.describe("Refusjon Omsorgspenger - Valideringer", () => {
  ...
  test("Steg 3: Validering av omsorgsdager", async ({ page }) => {
    ...
    const fomDate = `15.06.${previousYear}`;
    const tomDate = `20.06.${previousYear}`;
  ...
  test("Steg 3: Validering av fravær hele dager", async ({ page }) => {
    ...
    const fomDate = `15.01.${previousYear}`;
    ...
    const invalidTomDate = `15.01.${previousYear + 1}`;
    ...
    await expect(
      page.getByText(
        `Fraværet må være mellom 01.01.${previousYear} og ${formatDatoKort(endOfYear)}`,
      ),
    ).toBeVisible();

    // Fix the error by filling valid dates - errors should disappear
    const validTomDate = `20.01.${previousYear}`;
  ...
  test("Steg 3: Validering av fravær deler av dagen", async ({ page }) => {
    ...
    const invalidDato = `15.01.${previousYear + 1}`;
    ...
    const validDato = `15.01.${previousYear}`;
  ...
  test("Steg 3: Validering av dager som skal trekkes", async ({ page }) => {
    ...
    const fomDate = `20.01.${previousYear}`;
    const tomDate = `15.01.${previousYear}`;
    ...
    const fomDate2 = `20.01.${previousYear}`;
    const tomDate2 = `25.01.${previousYear}`;
  ...
  test("Steg 3: Validering av overlappende perioder", async ({ page }) => {
    ...
    await page
      ...
      .first()
      .click();
    const fom1 = `15.01.${previousYear}`;
    const tom1 = `20.01.${previousYear}`;
    ...
    await leggTilPerioderButtons.nth(1).click();
    const fom2 = `18.01.${previousYear}`;
    const tom2 = `22.01.${previousYear}`;
    ...
    const fom2Fixed = `25.01.${previousYear}`;
    const tom2Fixed = `28.01.${previousYear}`;
  ...
  test("Steg 3: Validering av fravær innenfor de 10 første dagene", async ({
    ...
    await page
      ...
      .first()
      .click();
    const fom = `05.01.${previousYear}`;
    const tom = `08.01.${previousYear}`;
    ...
    await page.getByLabel("Fra og med").first().fill(`20.01.${previousYear}`);
    await page.getByLabel("Til og med").first().fill(`25.01.${previousYear}`);
  ...
  test("Steg 4: Validering av inntekt", async ({ page }) => {
    ...
    await page
      ...
      .first()
      .click();
    const fom = `15.06.${previousYear}`;
    const tom = `20.06.${previousYear}`;
    await page.getByLabel("Fra og med").first().fill(fom);
    await page.getByLabel("Til og med").first().fill(tom);
    await page.getByRole("button", { name: "Neste steg" }).click();
  ...
  test("Steg 3: Validering av overlapp mellom fravær deler av dag og fravær hele dager", async ({
    ...
    await page
      ...
      .first()
      .click();
    const fom1 = `15.06.${previousYear}`;
    const tom1 = `20.06.${previousYear}`;
    await page.getByLabel("Fra og med").first().fill(fom1);
    await page.getByLabel("Til og med").first().fill(tom1);

    // Add fravær deler av dag that overlaps with fravær hele dager
    await page.getByRole("button", { name: "Legg til dag" }).click();
    const overlapDate = `18.06.${previousYear}`;
    ...
    const nonOverlapDate = `25.06.${previousYear}`;
    await page
  ...
  test("Steg 3: Validering av overlapp mellom dager som skal trekkes og fravær deler av dag", async ({
    ...
    await page.getByRole("radio", { name: "Ja" }).click();
    await page.getByRole("button", { name: "Legg til dag" }).click();
    const delerAvDagDate = `15.06.${previousYear}`;
    await page
      .getByRole("textbox", { name: "Dato" })
      .first()
      .fill(delerAvDagDate);
    ...
    // Fix the error by adjusting dates to not overlap - error should disappear and we can proceed
    const fomFixed = `25.06.${previousYear}`;
    const tomFixed = `28.06.${previousYear}`;
  ...
  test("Steg 3: Validering av overlappende perioder innenfor hele dager", async ({
    ...
    await page
      ...
      .click();
    await page.waitForTimeout(500);
    const fom2 = `18.06.${previousYear}`;
    const tom2 = `25.06.${previousYear}`;
    ...
    // Fix the error by adjusting dates to not overlap - error should disappear and we can proceed
    const fom2Fixed = `25.06.${previousYear}`;
    const tom2Fixed = `28.06.${previousYear}`;
  ...
  test("Steg 3: Validering av duplikate datoer innenfor delvise dager", async ({
    ...
    await page.getByRole("button", { name: "Legg til dag" }).click();
    await page.waitForTimeout(500);
    const dato1 = `15.06.${previousYear}`;
    ...
    const dato2Fixed = `20.06.${previousYear}`;
  ...
  test("Steg 4: Validering av korrigert inntekt og endringsårsaker", async ({
    ...
    const fomDate = `01.05.${previousYear}`;
    await page.getByLabel("Fra og med").fill(fomDate);
    await page.getByRole("button", { name: "Neste steg" }).click();
    ...
    // Fill tom before fom
    const tomDate = `01.04.${previousYear}`;
    ...
    const fomDateFixed = `01.05.${previousYear}`;
    const tomDateFixed = `15.05.${previousYear}`;
